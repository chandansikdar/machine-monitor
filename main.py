        ...
        if analysis_type == "Operational Schedule Compliance":
            ...
            for _idx, _a in enumerate(anomalies):
                ...
                _impact_html = (
                    ...
                    f'<p style="margin:0 0 4px 0;font-weight:700;color:#177E40;font-size:0.9em">'
                    f'💰 Potential impact</p>'
                    + (_impact if _impact.strip().startswith("<") else
        ...
        if analysis_type == "Operational Schedule Compliance":
            ...
            try:
                ...
                if _chart_data is not None and not _chart_data.empty:
                    ...
                    for _col in _numeric:
                        _s_all = _cd[_col].dropna()
                        _s_run = _cd_run[_col].dropna() if not _cd_run.empty else _s_all
                        if len(_s_run) < 4:
                            _s_run = _s_all
                        ...
                        if _col in _thresholds:
                            for _tk, _tc in [("warning", "#8E44AD"), ("critical", "#641E16")]:
                                ...
                                if _tv is not None:
                                    _fig.add_trace(_go.Scatter(
                                        x=[_xi_all[0], _xi_all[-1]], y=[_tv, _tv],
                                        mode="lines", line=dict(color=_tc, width=1.5, dash="dot"),
                                        name=f"{_tk.title()} threshold {_tv}", showlegend=True,
                                        hoverinfo="skip",
                                    ))
                        ...
                        # Actual data line
                        _fig.add_trace(_go.Scatter(
                            x=_xi_all, y=_cd[_col].values,
                            mode="lines", line=dict(color="#185FA5", width=1.2),
                            name=_col,
                            hovertemplate="%{x|%Y-%m-%d %H:%M}<br>" + _col + ": %{y:.3g}<extra></extra>",
                        ))
                ...
                import traceback
                st.error(f"Trend chart error: {_e}\n{traceback.format_exc()}")
    ...
    if data is None or data.empty:
        ...
        with left:
            ...
            if "Operational Schedule Compliance" in selected_analyses:
                # ── Initialise per-day schedule state ─────────────────────
                _DAYS = ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]
                _day_map = {"Mon":0,"Tue":1,"Wed":2,"Thu":3,"Fri":4,"Sat":5,"Sun":6}
                if "sched_per_day" not in st.session_state:
                    # Default: Mon-Fri 08:00-18:00, Sat-Sun off
